...
function search(query) {
  var url = host + '/search/sss?query=' + query + '&sort=rel';
  return (0, _requestPromise2['default'])(url).then(getSearchResults).then(detatchCheerioMetaData).then(convertToArray).then(siftTitles).then(function (results) {
    // console.log(results)
  });
}
...
function getPrice(metaLink) {
  var priceTag = metaLink.children[3].children[5].children[1].firstChild.data;
...
function getSearchResults(body) {
  ...
  return $('.row').map(function (i, post) {
    return {
      title: getTitle(post),
      url: getUrl(post),
      price: getPrice(post),
...
// post body
// contact info
var postURL = 'http://sfbay.craigslist.org/pen/cto/5193906043.html';

(0, _requestPromise2['default'])(postURL).then(getPostDetails);

function getPostBody($) {
  return $('#postingbody')[0].children.map(function (el) {
    return el.data;
  }).filter(function (el) {
